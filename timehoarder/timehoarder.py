...
def business_days(sdate:dt.datetime,edate:dt.datetime)->Generator[dt.datetime,None,None]:
...
def get_app_end(app):
    ...
    if app.end.date() == app.start.date():
        # if the start/end dates are on the same day, keep original end datetime
        end = app.end
    else:
        # otherwise manually set the end datetime to EOD on the start date
        naive_end = dt.datetime.combine(app.start.date(), dt.datetime.max.time())
...
def get_calendar(begin,end):
    ...
    calendar.Sort('[Start]')
    restriction = "[Start] >= '" + begin.strftime('%m/%d/%Y') + "' AND [END] <= '" + (end+dt.timedelta(days=1)).strftime('%m/%d/%Y') + "'"
...
def get_appointments(calendar):
    ...
    cal_duration = [(get_app_end(app) - app.start).total_seconds()/3600 for app in appointments]
...
def check_meeting_load(appointments):
    ...
    summary = appointments.groupby('date')['duration'].sum().to_frame().reset_index()
    ...
    return result[['date','weekday','duration']].style.background_gradient(axis=0)

def flag_overbooked(appointments, max_daily_load):
    """ flag days that are over the meeting max daily load (in hours) """
    if type(max_daily_load) == int:
        max_daily_dict = {d: max_daily_load for d in ["Monday","Tuesday","Wednesday","Thursday","Friday"]}
    else:
...
def hoard_time(overbooked, subject, hoard_hours=None, appointments=None,pref_am_pm=None):
    ...
    for date in (overbooked['date'].tolist()):
        if hoard_hours:
            ...
            df['diff'] = (df["start"] - df["end"].shift(1)).fillna(pd.Timedelta('0 days 00:00:00'))
            ...
            if block_candidates.shape[0]>0:
                if pref_am_pm == 'PM':
                    ...
                    ascorder = True
                hoard_data = (block_candidates.sort_values(by='start', axis=0, ascending=ascorder)).iloc[0]
                start_time = str((hoard_data['start']-hoard_data['diff']).time())
                duration_min = hoard_hours * 60
                print(f'Creating {hoard_hours} hour event starting at {start_time} on {date}')
            else:
